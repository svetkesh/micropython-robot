...
class RoboPad(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.joystickhand = Joystick(size_hint=(.7, .7),
                                     pos_hint={'x': .3, 'y': .1},  #)  #,
                                     sticky=True)
        ...
        # add some buttons
        self.firebutton = Button(size_hint=(.2, .2),
                                  pos_hint={'x': .15, 'y': .2},
                                  text='Fire!')
        ...
        self.slider_gear_factor = Slider(size_hint=(.3, .1),
                                                    pos_hint={'x': .15,
                                                              'y': .7},
                                                    min=2,
                                                    max=5,
        ...
        # Tank Control knob
        self.slider_runa = Slider(size_hint=(.1, .8),
                                  pos_hint={'x': .0,
                                            'y': .1},
                                  min=-98,
                                  max=98,
        ...
        self.add_widget(self.slider_runa)
        self.slider_runa.bind(value=self.OnSliderRunaValueChange)

        self.slider_runb = Slider(size_hint=(.1, .8),
                                  pos_hint={'x': .9,
                                            'y': .1},
                                  min=-98,
                                  max=98,
                                  value=0,
                                  orientation='vertical')
        self.add_widget(self.slider_runb)
        self.slider_runb.bind(value=self.OnSliderRunbValueChange)
    ...
    def OnSliderRunaValueChange(self, instance, value):
        # x = round(value)
        x = self.squaredround(value / 100, self.invert_runa)
    ...
    def OnSliderRunbValueChange(self, instance, value):
        y = self.squaredround(value / 100, self.invert_runb)
