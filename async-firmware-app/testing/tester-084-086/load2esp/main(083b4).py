...
0.8.3.0 measure and adjust Android app sensibility
"""
...
def run(host="192.168.4.1", port=80, loop_forever=True, backlog=16,  limit=2):
...
def driverobot(request):  # test to ensure command passes to robot driver
...
def robotlistener(request):
    print(request)


async def arobotlistener(request):  # try to async drive robot

    print('DBG running arobotlistener() {}'.format(request))
