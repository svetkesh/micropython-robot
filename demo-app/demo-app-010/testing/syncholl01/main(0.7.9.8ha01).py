...
ver 0.7.9.8ha01 - exhibition version, added Hall sensor connected to analogue conn
...
# hall_sensor = machine.Pin(4, machine.Pin.IN, machine.Pin.PULL_UP) # Hall sensor connected to D4 port2
hall_sensor = machine.ADC(0)  #ADC connector
# hall_sensor.value()
...
try:
    ...
    while True:
        networkpin.off()
        # print('DBG: hall_sensor.value(): {}, at : {}'.format(hall_sensor.value(), time.time())) # digital
        print('DBG: hall_sensor.read(): {}, '
              'at : {} '
              'since last loose: {}'.format(hall_sensor.read(),
                                            time.time(),
